...
class BNInception(nn.Module):
    def __init__(self, model_path='model_zoo/bninception/bn_inception.yaml', num_classes=101,
        ...
        src_path = os.getcwd()
        unmatch_layers = []
        with open(os.path.join(src_path, 'model_zoo', 'bninception', 'inception_unmatched_layers.txt')) as f:
            for l in f:
                unmatch_layers.append(l.strip())
        
        state_dict = torch.utils.model_zoo.load_url(weight_url)
        for name, weight in state_dict.items():
            if name in unmatch_layers:
                state_dict[name] = weight.squeeze(0)
